...
export default {
  ...
  methods: {
    ...
    playMusic() {
      ...
      if (this.musicIndex == 0) {
        axios
          .put(
            "/parties/event/" + this.$route.params.id + "/state",
            {
              state: 1,
            },
            {
  ...
  computed: {
    ...mapState({
      ...
      last_music: (state) => state.User.lastMusic,
    }),
  },
  mounted() {
    this.loadMusicList();
    if (this.last_music.event == this.$route.params.id) {
      this.musicIndex = this.last_music.index;
    }
  },
};
</script>
